...
dados = pd.read_csv(filename, skiprows=8, sep=";", encoding="UTF8", on_bad_lines="skip")
...
for index, row in dados.iterrows():
    ...
    valor_chuva_planilha = row.get('PRECIPITAÇÃO TOTAL, HORÁRIO (mm)', None)
    if valor_chuva_planilha is None:
        print("Chave não encontrada ou valor é None.")
        valor_chuva_planilha = 0.0
    else:
        try:
            valor_chuva_planilha = str(valor_chuva_planilha).replace(",", ".")
